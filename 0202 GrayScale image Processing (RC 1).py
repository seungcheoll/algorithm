...
def saveImage():
    global window, canvas, paper, filename
    global inImage, outImage, inH, inW, outH, outW
    saveFp = asksaveasfile(parent=window,mode='wb',defaultextension=".png", filetypes=(("칼라", "*.png;*.jpg;*.bmp;*.gif"),("모든 파일", "*.*"))) #바이너리 파일4
    new=Image.new('RGB', (outW, outH))
    for i in range(outH):
        for k in range(outW):
            r = outImage[0][i][k]
            g = outImage[1][i][k]
            b = outImage[2][i][k]
            new.putpixel((k, i), (r, g, b))
    new.save(saveFp)
    ...
    for i in range(inH) :
        for k in range(inW) :
            if (0 <= i+yVal< outH) and(0 <= k+xVal < outW):
                outImage[i+yVal][k+xVal] = inImage[i][k]
    ##############################
    displayImage()
